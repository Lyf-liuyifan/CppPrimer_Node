...
重载（Overloading）、隐藏（Hiding）和覆盖（Overriding）是面向对象编程中常见的三个概念，它们在C++中有着不同的含义和用法。

1. **重载（Overloading）：**
   - 重载是指在同一个作用域内，可以定义多个函数或操作符具有相同的名称，但是它们的参数列表或参数类型不同。
   - 在重载中，编译器会根据函数调用时提供的参数类型和数量来选择合适的函数进行调用。
   - 重载的函数或操作符可以有不同的返回类型。
   - 重载可以应用于成员函数、全局函数和操作符。
2. **隐藏（Hiding）：**
   - 隐藏是指派生类中的成员函数屏蔽了基类中的同名成员函数。如果在派生类中定义了一个和基类中同名的函数，那么基类中的函数将被隐藏，即使它们的参数列表不同。
   - 隐藏发生在不同的作用域之间，通常是派生类作用域隐藏了基类作用域中的函数。
   - 如果想要在派生类中访问被隐藏的基类成员函数，可以使用作用域解析运算符 `::`。
3. **覆盖（Overriding）：**
   - 覆盖是指派生类中的函数重写了基类中的虚函数，提供了新的实现。
   - 覆盖发生在继承关系中，通常是派生类覆盖了基类中的虚函数。
   - ==覆盖的函数在基类和派生类中具有相同的名称、参数列表和返回类型，并且在基类中被声明为虚函数。==
   - 当通过基类指针或引用调用覆盖的虚函数时，实际执行的是派生类中的函数。

区别：

- 重载和覆盖都是针对函数的，而隐藏是针对类成员的。
- 重载是同一个作用域内的多个函数，参数列表或类型不同；隐藏是不同作用域的同名函数，派生类隐藏了基类中的同名函数；覆盖是基类中的虚函数在派生类中被重新实现。
- 重载是静态绑定，根据调用时的参数类型和数量来确定调用哪个函数；覆盖是动态绑定，根据对象的实际类型来确定调用哪个函数。

综上所述，重载、隐藏和覆盖是不同概念，但它们都涉及到同名函数的处理，通过重载、隐藏和覆盖，可以实现不同的行为和语义。
...
**运行时多态**：在一些场合下，编译器无法在编译过程中完成联编，必须在程序运行时完成选择，因此编译器必须提供这么一套称为“动态联编”（dynamic binding）的机制，也叫动态联编。<font color=red>**C++通过虚函数来实现动态联编**</font>。接下来，我们提到的多态，不做特殊说明，指的就是<font color=red>**动态多态**</font>。
...
得到的结果，==基类的部分并没有消失，基类指针传入派生对象也会指向基类子类部分==
...
（1）覆盖是在虚函数之间的概念，需要派生类对象中定义的虚函数与基类中定义的虚函数的形式<span style=color:red;background:yellow>**完全相同**</span>；
...
在Parent的析构函数执行时，此时Son的析构函数已经执行完了，所以Parent的析构函数**也只能看到本层及以上的部分**。
...
第四次调用的结果是因为虚函数不是把原来的函数消灭了而是又创建了一个虚表去动态指向不同实现形式，所以第四次结果还是base的
...
>
> <img src="8.多态.assets/undefined202403201020011.png" alt="image-20240320102005961" style="zoom:67%;" />
>
> 
>
...
> 总结：
>
> - 如果通过对象来调用虚函数，那么不会通过虚表来找虚函数，因为编译器从一开始就确定调用函数的对象是什么类型，直接到程序代码区中找到对应函数的实现；==通过指针才满足多态条件==
> - 如果基类指针指向派生类对象，通过基类调用虚函数，若派生类中对这个虚函数进行了覆盖（重写-override），那么<font color=red>**符合动态多态的触发机制**</font>，最终的效果是基类指针调用到了派生类定义的虚函数；如果派生类对这个虚函数没有进行覆盖，==也会通过虚表访问，访问到的是基类自己定义的虚函数的入口地址==；
...
<img src="8.多态.assets/undefined202403201211237.png" alt="image-20240320121100154" style="zoom:67%;" />

因为d不是虚继承的b1,b2所以会把新定义的虚函数放到第一张虚表中。

==虚继承的会在最下面，继承是按照声明顺序来布局的==
